...
ISet.MaxAngleBetweenInterpolations = 0.2 -- degrees
...
do -- Add Toblerone(TM) Sleepers
	...
	m.ForcePaint = true
	m.PaintZones:Add(Color255(90, 43, 27, 255))
...
do --Create Gravel Bed
	local extrusion	= Gravelbed(Gravelbed.Presets.Gravel, ISet)
	spec.Extrusions:Add(extrusion)
end

do -- Create Embankment
	-- Pass nil to settings argument to use defaults.
	local extrusion = Embankment(nil, ISet)
	spec.Extrusions:Add(extrusion)
end
